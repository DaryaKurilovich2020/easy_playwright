...
import {
  INVALID_LOGIN_DATA,
  VALID_LOGIN_DATA,
} from "../../test-data/ui/login.testdata";
import { MainPage } from "../../ui/pages/MainPage";

test.describe("Login tests", () => {
  test.describe("Positive tests", () => {
    test("should login as basic user @smoke", async ({ loginPage, page }) => {
      ...
      await expect(
        mainPage.getByText("EasyRPA Control Server", { exact: false }),
      ).toBeVisible();
  ...
  test.describe("Negative tests", () => {
    INVALID_LOGIN_DATA.forEach((cred) => {
      test(`should return error on login attempt for user: ${cred.description}"`, async ({
        loginPage,
      }) => {
        await loginPage.login(cred.username, cred.password);
        await expect(loginPage.errorBlock).toContainText(
